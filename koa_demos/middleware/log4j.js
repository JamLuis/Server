...
async function logInfo(ctx, next) {
  try {
    logger.info(`[request path]:${ctx.path}, [request time]:${moment().format('YYYY-MM-DD HH:mm:ss')}, [ip]:${ctx.request.ip}`);
    await next();
  } catch (error) {
    logger.error(`[request path]:${ctx.path}, [request time]:${moment().format('YYYY-MM-DD HH:mm:ss')}, [error]:${JSON.stringify(error)}`)
  }
}
